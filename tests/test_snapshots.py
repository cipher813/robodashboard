...
def test_write_snapshot_idempotent_per_day(tmp_path):
    d = tmp_path / "snapshots"
    snapshots.write_snapshot(_df(nav_price=110.0), spy_close=500.0, source="live", snapshots_dir=d, today=date(2026, 4, 7))
    # Same day, updated figures → replaces, not duplicates.
    h = snapshots.write_snapshot(_df(nav_price=115.0), spy_close=505.0, source="live", snapshots_dir=d, today=date(2026, 4, 7))
    assert len(h) == 1
